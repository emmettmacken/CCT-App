  ...
  const fetchProfile = async () => {
    try {
      ...
      const { data, error } = await supabase
        .from("profiles")
        .select("name, age, trial_id, height, weight, consultant")
        .eq("id", user.id)
        .single();
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .update({
          ...
          age: tempProfile.age,
          trial_id: tempProfile.trialId,
          height: tempProfile.height,
          weight: tempProfile.weight,
          consultant: tempProfile.consultant,
        })
        .eq("id", user.id);
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.section}>
        ...
        {(
          ["name", "age", "trialId", "height", "weight", "consultant"] as (
            | keyof PatientProfile
          )[]
        ).map((field) => (
          <View key={field} style={styles.field}>
            ...
            {isEditing ? (
              <TextInput
                style={styles.input}
                value={tempProfile[field].toString()}
                onChangeText={(text) =>
                  handleInputChange(
                    field,
                    field === "name" ||
              ...
              <Text style={styles.value}>{tempProfile[field].toString()}</Text>
            )}
          </View>
        ))}
